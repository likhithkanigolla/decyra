...
var require_sasl = __commonJS({
  "node_modules/pg/lib/crypto/sasl.js"(exports2, module2) {
    ...
    function saslprep(password2) {
      ...
      return password2.replace(nonAsciiSpace, " ").replace(mappedToNothing, "").normalize("NFKC");
    }
    ...
    async function continueSession(session, password2, serverData, stream) {
      ...
      const saltedPassword = await crypto.deriveKey(saslprep(password2), saltBytes, sv.iteration);
...
var require_serializer = __commonJS({
  "node_modules/pg-protocol/dist/serializer.js"(exports2) {
    ...
    var password2 = (password3) => {
      return writer.addCString(password3).flush(
        112
        /* code.startup */
      );
    };
...
// src/integrations/database/local-auth.server.ts
...
// src/integrations/database/config.ts
...
// src/integrations/database/postgres.ts
var pool = null;
...
// src/integrations/database/local-auth.server.ts
var JWT_SECRET = process.env.POSTGRES_JWT_SECRET || "local-dev-jwt-secret-change-in-production";
var TOKEN_EXPIRY_SECONDS = 60 * 60 * 24 * 7;
function hashPassword(password2) {
  return new Promise((resolve, reject) => {
    const salt = (0, import_crypto.randomBytes)(16).toString("hex");
    (0, import_crypto.pbkdf2)(password2, salt, 1e5, 64, "sha256", (err, derivedKey) => {
      if (err) return reject(err);
      resolve(`${salt}:${derivedKey.toString("hex")}`);
    });
  });
}
async function createLocalUser(email2, password2, fullName2, role = "member") {
  const existingEmail = email2.toLowerCase().trim();
  ...
  try {
    const authUser = await queryOne(
      `INSERT INTO auth.users (email, encrypted_password, raw_user_meta_data)
       VALUES ($1, $2, $3)
       RETURNING id`,
      [existingEmail, passwordHash, JSON.stringify({ full_name: fullName2 })]
    );
    ...
    await query(
      `INSERT INTO public.local_users (id, email, full_name, password_hash, role)
       VALUES ($1, $2, $3, $4, $5)`,
      [userId, existingEmail, fullName2, passwordHash, role]
    );
    await query(
      `INSERT INTO public.profiles (id, email, full_name)
       VALUES ($1, $2, $3)
       ON CONFLICT (id) DO UPDATE SET email = EXCLUDED.email, full_name = EXCLUDED.full_name`,
      [userId, existingEmail, fullName2]
    );
    await query(
    ...
    await query("COMMIT");
    return { id: userId, email: existingEmail, full_name: fullName2, role };
  } catch (err) {
...
// src/integrations/database/migrate.ts
...
function getMigrationFiles(migrationsDir) {
  ...
  const files = import_fs.default.readdirSync(migrationsDir);
  return files.filter((f) => f.endsWith(".sql")).map((f) => ({
    name: f,
    path: import_path.default.join(migrationsDir, f),
...
async function getAppliedMigrations() {
  try {
    ...
    `);
    const migrations = await query("SELECT name FROM migrations");
...
async function executeMigration(filepath) {
  const sql = import_fs.default.readFileSync(filepath, "utf-8");
...
async function runMigrations(migrationsDir) {
  const dir = migrationsDir || import_path.default.resolve(__dirname, "../../../supabase/migrations");
  console.log(`\u{1F4C2} Looking for migrations in: ${dir}`);
  ...
  for (const migration of pending) {
    try {
      console.log(`  \u{1F4DD} Applying: ${migration.name}`);
...
if (process.argv[1] === __filename) {
  ...
  runMigrations(migrationsDir).then(() => {
    console.log("\u2705 Done");
    process.exit(0);
  }).catch((error) => {
...
async function main() {
  console.log("\u{1F504} Ensuring database is migrated...");
  await runMigrations();
  console.log(`
\u{1F464} Creating admin user:`);
...
`);
  try {
    ...
    console.log(`
\u{1F680} You can now log in at http://localhost:3000/auth`);
    console.log(`   Email   : ${email}`);
